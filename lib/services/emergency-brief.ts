...
import type { EmergencyBrief, Hospital } from '@/types/domain';

export async function buildEmergencyBrief(patientId: string): Promise<EmergencyBrief | null> {
  const [patientRes, vitalsRes, medsRes, alertRes, hospitalRes] = await Promise.all([
    supabaseAdmin.from('patient').select('*').eq('id', patientId).maybeSingle(),
    supabaseAdmin.from('vitals_log').select('*').eq('patient_id', patientId).order('logged_at', { ascending: false }).limit(7),
    supabaseAdmin.from('medication').select('*').eq('patient_id', patientId).eq('active', true),
    supabaseAdmin.from('alert').select('*').eq('patient_id', patientId).eq('status', 'open').order('created_at', { ascending: false }).limit(1),
    supabaseAdmin.from('hospital_mock').select('*')
  ]);
  ...
  const hospitalRows = hospitalRes.data ?? [];
  ...
  const hospitals: Hospital[] = (hospitalRows as any[])
    .map((h) => ({
      id: h.id,
      name: h.name,
  ...
  // Persist (best-effort)
  await supabaseAdmin.from('emergency_brief').insert({
